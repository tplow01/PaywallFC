...
// Monthly costs — Sky Sports + TNT Sports only.
// Both are required for the full season regardless of club.
// Amazon holds no PL rights in 2025-26.
export const PRICES = {
...
// Total cost so far — Sky + TNT + TV Licence
export function calcSoFar(skyMode, months) {
  const sky = skyMode === "now" ? PRICES.skyNow : PRICES.skyBundle;
  return (sky + PRICES.tnt) * months + tvLicSoFar(months);
}

// Minimum remaining cost — Sky + TNT + TV Licence for remaining months
export function calcRemaining(skyMode, months) {
  const remaining = SEASON_MONTHS - months;
  const sky       = skyMode === "now" ? PRICES.skyNow : PRICES.skyBundle;
  return (sky + PRICES.tnt) * remaining + tvLicRemaining(months);
}
...
export function isBlackout(utcDate) {
  ...
  const ukHour       = ukDate.getUTCHours();
  const ukMin        = ukDate.getUTCMinutes();

  // Blackout window: 14:45–17:15 UK time — match kicks off at exactly 15:00
  return isUKSaturday && ukHour === 15 && ukMin === 0;
}
